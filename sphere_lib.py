...
def koords_in_kegel_cache(range_r = 4, alpha = np.array([]), beta = np.array([])):
    ...
    for i, ca, sa, cb, sb, cacb, sacb, casb, sasb in tqdm(zip(range(len(cos_alpha)), cos_alpha, sin_alpha, cos_beta, sin_beta, cos_alpha_cos_beta, sin_alpha_cos_beta, cos_alpha_sin_beta, sin_alpha_sin_beta)):
...
def Get_AODF_noRand_noCache_amp(ODFs:np.ndarray, result:np.ndarray, basis:np.ndarray, weights:np.ndarray, phi:np.ndarray, theta:np.ndarray,x_koord:int, y_koord:int, z_koord:int, factor_amp:int = 1000, sigma:float=2):
    # Mit weights alle punkte im Kegel ablaufen
    result_ = np.copy(result)
    result_[:,0,:] += x_koord
    result_[:,1,:] += y_koord
