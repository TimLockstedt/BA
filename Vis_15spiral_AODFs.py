...
def genereate_divergence_(middle_x:int=7,middle_y:int=7,middle_z:int=7):
    field_phi = np.empty((2*range_r+9,2*range_r+9,2*range_r+1))
...
for i in tqdm(range(AODFs.shape[2]),desc='Generiere Bilder', leave=False):
    ...
    plt.savefig(f"new_lib_15Spiral_{i+range_r}_AODF_b{bands}_s{int(sigma*10)}_c{coefficient}_famp{factor_amp}_n{number_of_winkel}_r{range_r}_fib_2.png", dpi=1000)
    plt.clf()
