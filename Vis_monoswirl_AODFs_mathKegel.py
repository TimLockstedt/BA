...
def genereate_divergence_(middle_x:int=4,middle_y:int=4,middle_z:int=4):
    field_phi = np.empty((11,11,2*range_r+3))
    field_theta = np.copy(field_phi)
    for i in range(field_phi.shape[0]):
        for j in range(field_phi.shape[1]):
            for k in range(field_phi.shape[2]):
...
AODFs = np.array([
    ...
    for i in tqdm(range(range_r+1, limit_x - range_r-1),desc='Schleife x', ascii="░▒█")
...
try:
    AODFs = np.reshape(AODFs,(limit_x - 2*range_r-2,limit_y - 2*range_r-2,limit_z - 2*range_r-2, odf.get_num_coeff(bands)))
except:
    print("AODFs reshape failed, error", AODFs.shape)
    shape_x = int(input("Bitte shape eingeben, erst x, dann y und z \n"))
    shape_y = int(input("y:\n"))
    shape_z = int(input("z:\n"))
...
for i in tqdm(range(AODFs.shape[2]),desc='Generiere Bilder', leave=False):
    ...
    plt.savefig(f"new_lib_MonoSwirl_{i+range_r}_AODF_b{bands}_s{int(sigma*10)}_c{coefficient}_famp{factor_amp}_n{number_of_winkel}_r{range_r}_abgKegel_big.png", dpi=500)
    plt.clf()
