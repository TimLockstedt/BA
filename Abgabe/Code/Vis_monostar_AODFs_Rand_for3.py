...
number_of_winkel_array = np.array([1500,5000,10000,20000,50000,100000,200000])
# Gaussfunktion Sigma
sigma = 0.3
...
for number_of_winkel in number_of_winkel_array:
    for count in range(10):
        ...
        for i in tqdm(range(AODFs.shape[2]),desc='Generiere Bilder', leave=False):
            ...
            plt.savefig(f"new_lib_MonoStar_{i+range_r}_AODF_b{bands}_s{int(sigma*10)}_c{coefficient}_famp{factor_amp}_n{number_of_winkel}_r{range_r}_Rand_{factor}_{count}.png", dpi=500)
            plt.clf()
