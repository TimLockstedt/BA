...
factor = 100
# Cache einlesen, falls vorher abgespeichert
dict_4_10, dict_basis = load_dict(range_r, factor), load_dict(range_r, bands) 

# Alternativ Cache erstellen
# dict_4_10, dict_basis = generate_dict_basis(range_r, factor, bands)
# Chache speichern, damit dieser nicht erneut berehnet werden muss
    ...
    for i in range(field_phi.shape[0]):
        for j in range(field_phi.shape[1]):
            for k in range(field_phi.shape[2]):
                field_phi[i,j,k] = np.arccos((k-7)/np.linalg.norm([i-7,j-7,k-(2*range_r+1)])) # np.arccos(k+20/np.linalg.norm([i+20,j+20,k+20]))
...
for i in tqdm(range(AODFs.shape[2]),desc='Generiere Bilder', leave=False):
    ...
    plt.savefig(f"new_lib_ministar_{i+range_r}_AODF_b{bands}_s{int(sigma*10)}_c{coefficient}_famp{factor_amp}_n{number_of_winkel}_r{range_r}_Rand_{factor}_2.png", dpi=500)
    plt.clf()
