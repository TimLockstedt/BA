...
for i in tqdm(range(AODFs.shape[2]),desc='Generiere Bilder', leave=False):
    ...
    plt.savefig(f"new_lib_MiniSwirl_offcenter5_{i+range_r}_AODF_b{bands}_s{int(sigma*10)}_c{coefficient}_famp{factor_amp}_n{number_of_winkel}_r{range_r}_fib_2_noInc.png", dpi=500)
    plt.clf()
