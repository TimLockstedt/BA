...
ODFs = odf3.compute(np.deg2rad(Direction_image)[:,:,:,None], np.deg2rad(Inclination_image)[:,:,:,None], mask_image[:,:,:,None], bands)[580:610,915:945,2:7,:]# [600:650,900:950,:,:]
...
for i in range(AODFs.shape[2]):
    ...
    img = mpimg.imread('MSA0309_M8_70mu_70ms_s0304_a00_d000_NTransmittance.png')
    plt.imshow(img[582:608,917:943], alpha=.6)
    plt.imshow(image, alpha=.6)
    plt.xticks([])
    plt.yticks([])
    plt.subplots_adjust(top = 1, bottom = 0, right = 1, left = 0, hspace = 0, wspace = 0)
    plt.savefig(f"new_lib_PLI_600-650_900-950_{300+i+range_r}_AODF_b{bands}_s{int(sigma*10)}_c{coefficient}_famp{factor_amp}_n{number_of_winkel}_r{range_r}_fib_2_withTransmittance1.png", dpi=500)
    plt.clf()
